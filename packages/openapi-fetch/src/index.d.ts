...
type SecuritySchemeToHeaders<T extends Record<string, SecuritySchemeObject> | undefined> = {
  [K in keyof T]: T[K] extends { in: "header"; name: string }
    ? { [name in T[K]["name"]]: string }
    : T[K] extends { scheme: "bearer" } | { in: "header" }
...
export type ParamsOption<T> = T extends {
  ...
  ? T extends { security: any }
    ? {
        params: Omit<T["parameters"], "header"> & {
          header: T["parameters"] extends { header: infer H }
            ? H & SecuritySchemeToHeaders<T["security"]>
            : SecuritySchemeToHeaders<T["security"]>;
        };
      }
...
// type pathtest = paths["/users/me"];
// type test3 = ParamsOption<pathtest["get"]>;

// type test = MaybeOptionalInit<pathtest, "get">;
// type test2 = InitParam<test>;
